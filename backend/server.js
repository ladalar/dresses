...
app.post('/api/dresses', (req, res) => {
  const { name, image_url, price, link, rank, comments } = req.body;
  if (!name) return res.status(400).json({ error: 'Name is required' });

  const result = db
    .prepare(
      'INSERT INTO dresses (name, image_url, price, link, rank, comments) VALUES (?, ?, ?, ?, ?, ?)'
    )
    .run(name, image_url || null, price || null, link || null, rank || null, comments || null);
...
app.put('/api/dresses/:id', (req, res) => {
  ...
  db.prepare(
    'UPDATE dresses SET name = ?, image_url = ?, price = ?, link = ?, rank = ?, comments = ? WHERE id = ?'
  ).run(
    name ?? existing.name,
